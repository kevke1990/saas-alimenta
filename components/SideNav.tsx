...
export default function SideNav() {
  ...
  return (
    <nav className="side-nav side-nav-premium" aria-label="Werkpleknavigatie">
      ...
      {nav.map((item) => {
        ...
        return <Link key={item.href} href={item.href} className={`side-link side-link-premium${active ? " active" : ""}`} aria-current={active ? "page" : undefined}><Icon name={item.icon}/><span>{item.label}</span>{item.href === "/cases/new" ? <span className="nav-cta-dot" /> : null}</Link>;
      })}
    </nav>
  );
}
