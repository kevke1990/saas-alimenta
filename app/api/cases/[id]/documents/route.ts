...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const documents = await db.document.findMany({ where: { caseId: id, userId: user.id }, orderBy: { createdAt: "desc" }, include: { incomeFacts: { orderBy: { createdAt: "asc" } } } });
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (text.length < 30 || text.length > AI_INCOME_MAX_INPUT_CHARS || process.env.AI_PROCESSING_DISABLED === "true") {
    ...
    const updated = await db.document.update({ where: { id: document.id }, data: { aiStatus } });
    return NextResponse.json({ ok: true, document: updated, warning: aiStatus === "DISABLED" ? "AI-verwerking is uitgeschakeld." : aiStatus === "TOO_LARGE" ? "Document opgeslagen; AI-analyse overgeslagen wegens de analysegrens." : "Te weinig tekst voor betrouwbare AI-extractie." }, { status: 201 });
  }
  ...
  try {
    ...
    const facts: Array<[string,string,number,string]> = [
      ["grossAnnual", "Bruto jaarinkomen", result.grossAnnual, "EUR/jaar"], ["holidayAllowance", "Vakantiegeld", result.holidayAllowance, "EUR/jaar"],
      ["thirteenthMonth", "13e maand", result.thirteenthMonth, "EUR/jaar"], ["ikb", "IKB", result.ikb, "EUR/jaar"],
      ["pensionPremium", "Pensioenpremie", result.pensionPremium, "EUR/jaar"], ["taxableIncome", "Belastbaar inkomen", result.taxableIncome, "EUR/jaar"], ["netAnnual", "Netto jaarinkomen", result.netAnnual, "EUR/jaar"],
    ].filter(([, , value]) => typeof value === "number" && Number.isFinite(value)) as Array<[string,string,number,string]>;
