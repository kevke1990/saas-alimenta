...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){const user=await requireUser();const{id}=await params;const access=await requireCaseTenantAccess(user.id,id,"PROFESSIONAL");const c=await db.case.findFirst({where:{id,organizationId:access.organizationId,deletedAt:null}});if(!c)return new NextResponse('Dossier niet gevonden.',{status:404});return NextResponse.json(await db.professionalOverride.findMany({where:{caseId:id},orderBy:{createdAt:'desc'}}));}

export async function POST(req:Request,{params}:{params:Promise<{id:string}>}){
 try{
  const user=await requireUser();const{id}=await params;const access=await requireCaseTenantAccess(user.id,id,"PROFESSIONAL");const isJson=req.headers.get('content-type')?.includes('application/json');
  const c=await db.case.findFirst({where:{id,organizationId:access.organizationId,deletedAt:null,status:{not:'ARCHIVED'}},include:{calculations:{orderBy:{createdAt:'desc'},take:1}}});
