...
const ALLOWED_RESULT_FIELDS = new Set([
  'combined.childSupportTotal',
  'combined.childSupportByParent.0',
  'combined.childSupportByParent.1',
  'combined.partnerSupport.monthlyNet',
  'combined.partnerSupport.monthlyGross',
  'combined.totalMonthlyPayments',
  'partnerSupport.result.monthlyNet',
  'partnerSupport.result.monthlyGross',
]);
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const b: any = req.headers.get('content-type')?.includes('application/json') ? await req.json() : Object.fromEntries((await req.formData()).entries());
    ...
    if (!current) return new NextResponse('Er is nog geen berekeningssnapshot beschikbaar.', { status: 409 });
    const originalValue = readPath(current.result, v.field);
    const adjustedResult = writePath(current.result, v.field, v.overrideValue);
    const fingerprint = professionalAdjustmentFingerprint(String((current.result as any)?.calculationFingerprint || ''), { ...v, originalValue });
    ...
    const updated = await db.$transaction(async (tx) => {
      const created = await tx.professionalOverride.create({ data: { caseId: id, userId: user.id, field: v.field, originalValue: originalValue as any, overrideValue: v.overrideValue as any, reason: v.reason } });
      const calculation = await tx.calculation.create({ data: { caseId: id, engineVersion: current.engineVersion, normVersion: current.normVersion, inputSnapshot: current.inputSnapshot, result: finalResult as any } });
      const nextCase = await tx.case.update({ where: { id }, data: { result: finalResult as any, status: 'CALCULATED', reviewStatus: 'INCOMPLETE', reviewedAt: null, approvedAt: null, approvedByUserId: null } });
      await tx.auditLog.create({ data: { userId: user.id, action: 'PROFESSIONAL_OVERRIDE_CREATED', metadata: { caseId: id, overrideId: created.id, field: v.field, calculationId: calculation.id, previousCalculationId: current.id, originalValue, overrideValue: v.overrideValue, reason: v.reason, fingerprint } } });
      await tx.auditLog.create({ data: { userId: user.id, action: 'CASE_RECALCULATED_WITH_OVERRIDE', metadata: { caseId: id, calculationId: calculation.id, previousCalculationId: current.id, fingerprint, reviewReset: true } } });
