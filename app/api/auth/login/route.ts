...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (u.mfaEnabled) {
      if (!u.mfaSecretCipher || !code || !verifyTotp(decryptSecret(u.mfaSecretCipher), String(code))) return new NextResponse("MFA-code vereist of ongeldig", { status: 401 });
    }
    await createSession(u.id);
    await db.user.update({ where: { id: u.id }, data: { lastLoginAt: new Date() } });
    return NextResponse.json({ ok: true, mfa: u.mfaEnabled });
  } catch {
