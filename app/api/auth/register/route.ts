...
export async function POST(req: Request) {
  ...
  try {
    ...
    const user = await db.user.create({ data: { email: body.email.toLowerCase(), passwordHash: await hashPassword(body.password), name: body.name, companyName: body.companyName || null } });
