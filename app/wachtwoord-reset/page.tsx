...
export default function PasswordResetPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setDone(true); setTimeout(() => router.push("/login"), 1200);
    } catch (e: any) { setError(e?.message || "Resetten is mislukt."); }
    finally { setBusy(false); }
  }
  return <main className="auth-card-wrap" style={{ minHeight: "100vh" }}><section className="auth-card">
    <h2>Nieuw wachtwoord</h2>
    {done ? <><div className="notice">Je wachtwoord is gewijzigd. Je wordt doorgestuurd naar de loginpagina.</div><p className="topgap"><Link href="/login" style={{color:"#315efb",fontWeight:700}}>Nu inloggen</Link></p></> : <>
      <p>Kies een nieuw wachtwoord van minimaal 12 tekens.</p>
      {error && <div className="notice error topgap">{error}</div>}
      <form onSubmit={submit} className="topgap"><div><label className="label">Nieuw wachtwoord</label><input className="input" type="password" required minLength={12} autoComplete="new-password" value={password} onChange={e=>setPassword(e.target.value)}/></div><div className="topgap"><label className="label">Herhaal wachtwoord</label><input className="input" type="password" required minLength={12} autoComplete="new-password" value={confirm} onChange={e=>setConfirm(e.target.value)}/></div><button className="btn topgap" disabled={busy} style={{width:"100%",justifyContent:"center"}}>{busy?"Opslaan…":"Wachtwoord opslaan"}</button></form>
    </>}
  </section></main>;
}
