...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return <html lang="nl"><body>{process.env.DEMO_MODE === "true" ? <div style={{position:"sticky",top:0,zIndex:9999,background:"#163C3A",color:"white",padding:"8px 12px",textAlign:"center",fontSize:13}}>DEMO-MODUS — fictieve gegevens — niet gebruiken voor echte dossiers</div> : null}{children}<script dangerouslySetInnerHTML={{__html:'if("serviceWorker" in navigator){window.addEventListener("load",()=>navigator.serviceWorker.register("/sw.js").catch(()=>{}))'}} /></body></html>;
}
