import Link from "next/link";
import AppShell from "@/components/AppShell";
import CaseActions from "./CaseActions";
import { requireUser } from "@/lib/auth";
import { db } from "@/lib/db";

const money = (v: any) => new Intl.NumberFormat("nl-NL", { style: "currency", currency: "EUR", maximumFractionDigits: 0 }).format(Number(v) || 0);
const date = (v: any) => new Intl.DateTimeFormat("nl-NL", { day: "2-digit", month: "short", year: "numeric" }).format(new Date(v));
const reviewLabel = (v: string) => v === "FINAL" ? "Definitief" : v === "APPROVED" ? "Goedgekeurd" : v === "REVIEWED" ? "Gereviewd" : v === "READY_FOR_REVIEW" ? "Klaar voor review" : v === "IN_REVIEW" ? "In review" : "Te controleren";
const reviewClass = (v: string) => v === "FINAL" || v === "APPROVED" ? "green" : v === "REVIEWED" || v === "READY_FOR_REVIEW" || v === "IN_REVIEW" ? "amber" : "gray";

type SearchParams = Promise<{ q?: string; review?: string }>;

export default async function CasesPage({ searchParams }: { searchParams?: SearchParams }) {
  const u = await requireUser();
  const params = searchParams ? await searchParams : {};
  const q = String(params.q || "").trim();
  const allowedReviews = ["INCOMPLETE", "READY_FOR_REVIEW", "REVIEWED", "APPROVED", "FINAL"];
  const review = allowedReviews.includes(String(params.review)) ? String(params.review) : "";

  const cases = await db.case.findMany({
    where: {
      userId: u.id,
      status: { in: ["DRAFT", "CALCULATED"] },
      ...(review ? { reviewStatus: review } : {}),
      ...(q ? {
        OR: [
          { name: { contains: q, mode: "insensitive" } },
          { client: { name: { contains: q, mode: "insensitive" } } },
          { client: { reference: { contains: q, mode: "insensitive" } } },
        ],
      } : {}),
    },
    include: { client: true, calculations: { orderBy: { createdAt: "desc" }, take: 1 } },
    orderBy: { updatedAt: "desc" },
  });

  return <AppShell>
    <div className="page-head"><div><div className="eyebrow">Werkplek · dossiers</div><h1 className="page-title">Dossiers</h1><p className="page-subtitle">Alle actieve alimentatieberekeningen, met versiehistorie en professionele reviewstatus.</p></div><Link className="btn" href="/cases/new">+ Nieuwe berekening</Link></div>
    <section className="panel">
      <form method="get" className="filters" style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "end", marginBottom: 18 }}>
        <label style={{ flex: "1 1 300px" }}><span className="field-label">Zoeken</span><input className="input" name="q" defaultValue={q} placeholder="Dossier, cliënt of klantnummer" /></label>
        <label style={{ flex: "0 1 220px" }}><span className="field-label">Reviewstatus</span><select className="input" name="review" defaultValue={review}><option value="">Alle statussen</option>{allowedReviews.map(v => <option key={v} value={v}>{reviewLabel(v)}</option>)}</select></label>
        <button className="btn" type="submit">Filteren</button>
        {(q || review) && <Link className="btn secondary" href="/cases">Reset</Link>}
      </form>
      <div className="panel-head"><div><h2 className="panel-title">Dossieroverzicht</h2><div className="panel-sub">{cases.length} {cases.length === 1 ? "dossier" : "dossiers"}</div></div></div>
      {cases.length === 0 ? <div className="empty-state"><h3>Geen dossiers gevonden</h3><p>Pas de filters aan of start een nieuwe berekening.</p><Link className="btn" href="/cases/new">Nieuwe berekening</Link></div> :
        <div className="table-wrap"><table className="table"><thead><tr><th>Dossier</th><th>Cliënt</th><th>Berekening</th><th>Review</th><th>Norm / engine</th><th>Bijgewerkt</th><th></th></tr></thead><tbody>{cases.map(c => { const r: any = c.result || {}; const calc = c.calculations[0]; return <tr key={c.id}><td><Link className="table-link" href={`/cases/${c.id}`}>{c.name}</Link></td><td>{c.client?.name || "—"}</td><td>{money(r.totalNeed)}</td><td><Link href={`/cases/${c.id}/review`}><span className={`status ${reviewClass(c.reviewStatus)}`}>{reviewLabel(c.reviewStatus)}</span></Link></td><td>{calc ? `${calc.normVersion} · ${calc.engineVersion}` : c.calculationVersion}</td><td>{date(c.updatedAt)}</td><td><CaseActions caseId={c.id} locked={c.reviewStatus === "FINAL"} /></td></tr> })}</tbody></table></div>}
    </section>
  </AppShell>;
}
