...
export default async function WorkPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const scored = cases.map(c => {
    const facts = c.documents.flatMap(d => d.incomeFacts); const proposed = facts.filter(f => f.status === "PROPOSED").length;
  ...
  return <AppShell>
    <div className="page-head"><div><div className="eyebrow">Werkplek · actiecentrum</div><h1 className="page-title">Mijn werkvoorraad</h1><p className="page-subtitle">Eén overzicht van dossiers die aandacht vragen, open taken en controlepunten.</p></div><div className="actions"><Link className="btn secondary" href="/cases">Alle dossiers</Link><Link className="btn" href="/tasks">Open taken</Link></div></div>
    <section className="result-overview"><div className="result-primary"><div className="stat-label">ACTIEF WERK</div><div className="result-amount">{counts.URGENT + counts.HIGH}</div><div className="stat-meta">urgente en hoge prioriteit dossiers</div></div><div className="result-metric"><div className="stat-label">URGENT</div><div className="metric-value">{counts.URGENT}</div><span>direct oppakken</span></div><div className="result-metric"><div className="stat-label">HOOG</div><div className="metric-value">{counts.HIGH}</div><span>deze werkstroom</span></div><div className="result-metric"><div className="stat-label">OPEN TAKEN</div><div className="metric-value">{tasks.length}{overdue > 0 ? <small style={{ marginLeft: 6, color: "#b91c1c" }}>· {overdue} te laat</small> : null}</div><span>professionele opvolging</span></div></section>
    <section className="panel topgap"><div className="panel-head"><div><div className="section-kicker">Prioriteitswachtrij</div><h2 className="panel-title">Dossiers met het meeste werk eerst</h2><div className="panel-sub">De score is een uitlegbare workflowhint en verandert nooit de juridische berekening.</div></div></div><form method="get" className="filters" style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "end", marginBottom: 18 }}><label><span className="field-label">Prioriteit</span><select className="input" name="priority" defaultValue={priority}><option value="">Alle prioriteiten</option>{allowedPriorities.map(p => <option key={p} value={p}>{priorityLabel(p)}</option>)}</select></label><button className="btn" type="submit">Filteren</button>{priority && <Link className="btn secondary" href="/work">Reset</Link>}</form>{visible.length === 0 ? <div className="empty">Geen dossiers met deze prioriteit.</div> : <div className="table-wrap"><table className="table"><thead><tr><th>Dossier</th><th>Cliënt</th><th>Prioriteit</th><th>Review</th><th>Reden</th><th>Bijgewerkt</th><th></th></tr></thead><tbody>{visible.slice(0, 50).map(({ c, score }) => <tr key={c.id}><td><Link className="table-link" href={`/cases/${c.id}/workspace`}>{c.name}</Link></td><td>{c.client?.name || "—"}</td><td><span className={`status ${priorityTone(score.priority)}`}>{score.score}/100 · {priorityLabel(score.priority)}</span></td><td>{reviewLabel(c.reviewStatus)}</td><td><div className="table-note">{score.reasons[0] || "Geen openstaand controlepunt."}</div>{score.reasons.length > 1 && <div className="table-note">+{score.reasons.length - 1} andere</div>}</td><td>{date(c.updatedAt)}</td><td><Link className="btn secondary" href={`/cases/${c.id}/intelligence`}>Details</Link></td></tr>)}</tbody></table></div>}</section>
    <section className="panel topgap"><div className="panel-head"><div><div className="section-kicker">Opvolging</div><h2 className="panel-title">Open taken</h2><div className="panel-sub">Taken blijven expliciet en auditbaar; afronden vereist een bewuste professionele actie.</div></div><Link className="btn secondary" href="/tasks">Volledige werkvoorraad →</Link></div>{tasks.length === 0 ? <div className="empty">Geen open taken.</div> : <div className="detail-grid">{tasks.slice(0, 6).map(task => <div className="detail-card" key={task.id}><div className="detail-card-head"><b>{task.title}</b><span className={`status ${task.priority === "URGENT" ? "red" : task.priority === "HIGH" ? "amber" : "gray"}`}>{priorityLabel(task.priority)}</span></div><p className="subtle">{task.description || "Geen omschrijving."}</p><div className="summary-line"><span>Dossier</span><b>{task.case ? task.case.name : "Algemene taak"}</b></div><div className="summary-line"><span>Deadline</span><b>{task.dueAt ? date(task.dueAt) : "Geen deadline"}</b></div><TaskActions taskId={task.id} /></div>)}</div>}</section>
    <section className="panel topgap"><div className="panel-head"><div><div className="section-kicker">Werkverdeling</div><h2 className="panel-title">Prioriteitsoverzicht</h2></div></div><div className="detail-grid"><div className="detail-card"><div className="detail-card-head"><b>Urgent</b><span>{counts.URGENT} dossiers</span></div><p className="subtle">Ontbrekende berekeningen, fouten of verlopen opvolging krijgen voorrang.</p></div><div className="detail-card"><div className="detail-card-head"><b>Hoog</b><span>{counts.HIGH} dossiers</span></div><p className="subtle">Open review, goedkeuring of substantiële dossierwerkzaamheden.</p></div><div className="detail-card"><div className="detail-card-head"><b>Normaal</b><span>{counts.NORMAL} dossiers</span></div><p className="subtle">Werk dat gepland kan worden binnen de normale werkstroom.</p></div><div className="detail-card"><div className="detail-card-head"><b>Laag</b><span>{counts.LOW} dossiers</span></div><p className="subtle">Dossiers zonder relevante openstaande professionele acties.</p></div></div></section>
  </AppShell>;
}
