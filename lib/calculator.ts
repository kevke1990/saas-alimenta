...
function validate(input: CaseInput) { if (!input || !Array.isArray(input.parents) || input.parents.length !== 2) throw new Error("Er moeten precies twee onderhoudsplichtige ouders worden ingevoerd."); if (input.priorContribution !== undefined && (!Number.isFinite(input.priorContribution) || input.priorContribution < 0)) throw new Error("Bestaande bijdrage voor indexering is ongeldig."); if (input.applyIndexation && input.priorContribution === undefined) throw new Error("Voor indexering moet eerst een bestaande bijdrage worden vastgelegd."); if (!Array.isArray(input.children) || input.children.length === 0) throw new Error("Minimaal één kind vereist."); if (input.children.length > 10) throw new Error("Maximaal 10 kinderen per berekening."); for (const [i, p] of input.parents.entries()) { if (!Number.isFinite(p.nbi) || p.nbi < 0) throw new Error("NBI ouder " + (i + 1) + " is ongeldig."); if ((p.careDaysPerWeek ?? 0) < 0 || (p.careDaysPerWeek ?? 0) > 7) throw new Error("Zorgdagen ouder " + (i + 1) + " moeten tussen 0 en 7 liggen."); } }
