...
export async function createTenantInvite(input: {
  ...
  assertTenantRole(input.role ?? "PROFESSIONAL", "READ_ONLY");
  ...
  await db.$executeRaw(Prisma.sql`
    ...
    VALUES (${id}, ${input.organizationId}, ${input.invitedByUserId}, ${input.email.trim().toLowerCase()}, ${input.role ?? "PROFESSIONAL"}, ${tokenHash}, ${expiresAt})
  `);
  return { id, token, expiresAt, email: input.email.trim().toLowerCase(), role: input.role ?? "PROFESSIONAL" };
}
...
export async function acceptTenantInvite(token: string, userId: string) {
  ...
  return db.$transaction(async tx => {
    ...
    await tx.$executeRaw(Prisma.sql`
      ...
      ON CONFLICT ("userId") DO UPDATE SET "organizationId" = EXCLUDED."organizationId", "role" = EXCLUDED."role", "updatedAt" = CURRENT_TIMESTAMP
    `);
