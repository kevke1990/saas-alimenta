...
export function calculateWorkScore(input: WorkScoreInput): WorkScore {
  ...
  const reviewPenalty: Record<string, number> = {
    INCOMPLETE: 10,
    READY_FOR_REVIEW: 25,
    REVIEWED: 15,
    APPROVED: 5,
  ...
  if (input.proposedIncomeFacts > 0) {
    score -= Math.min(20, input.proposedIncomeFacts * 4);
    reasons.push(`${input.proposedIncomeFacts} inkomensfeit(en) wachten op goedkeuring.`);
  }
  if (input.documentsAwaitingReview > 0) {
    score -= Math.min(15, input.documentsAwaitingReview * 3);
    reasons.push(`${input.documentsAwaitingReview} document(en) wachten op review.`);
  }
  if (input.documentAnalysisErrors > 0) {
    score -= Math.min(25, input.documentAnalysisErrors * 10);
    reasons.push(`${input.documentAnalysisErrors} documentanalyse(s) bevatten een fout.`);
  }
  if (input.calculationCount === 0) {
    score -= 25;
    reasons.push("Er is nog geen berekeningssnapshot.");
  }
  if (input.calculationStale) {
    score -= 15;
    reasons.push("De invoer is gewijzigd na de laatste berekening.");
  }
