...
export function verificationEmailTemplate(input: { name?: string | null; url: string }) {
  ...
  return {
    subject: "Bevestig je e-mailadres voor Merelo",
    ...
    htmlBody: `<p>${safeGreeting}</p><p>Bevestig je e-mailadres voor Merelo.</p><p><a href="${safeUrl}">E-mailadres bevestigen</a></p><p>Deze link is 24 uur geldig.</p>`,
  };
}
...
export function passwordResetEmailTemplate(input: { name?: string | null; url: string }) {
  ...
  const safeUrl = escapeHtml(input.url);
  return {
    subject: "Wachtwoord resetten voor Merelo",
