export type ProfessionalReport = {
  ...
  summary: {
    ...
    partnerSupportGrossMonthly: number;
    partnerSupportNetMonthly: number;
  ...
  audit: {
    priorityAudit?: Record<string, unknown>;
    warnings: string[];
    disclaimer: string;
  };
};
...
export function buildProfessionalReport(input: any): ProfessionalReport {
  ...
  return {
    ...
    caseName: String(input.name || 'Onbenoemd dossier'),
    clientName: input.client?.name || undefined,
    ...
    summary: {
      ...
      totalNeed: num(result.totalNeed),
      totalCapacity: num(result.totalCapacity),
      capacityDeficit: num(result.capacityDeficit),
    },
    children: children.map((child: any, i: number) => ({
      index: i + 1,
      name: child.name || `Kind ${i + 1}`,
      age: child.age,
      residence: child.residence,
      specialCosts: num(child.specialCosts),
      ownIncome: num(child.ownIncome),
    })),
    parents: parents.map((parent: any, i: number) => ({
      index: i + 1,
      name: parent.name || `Ouder ${String.fromCharCode(65 + i)}`,
      nbi: num(parent.nbi),
      kgb: num(parent.kgb),
      housingCosts: num(parent.housing?.monthlyCosts ?? parent.housingCosts),
      careDaysPerWeek: num(parent.careDaysPerWeek),
    })),
